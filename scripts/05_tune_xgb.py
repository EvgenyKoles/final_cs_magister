...
Notes:
- RepeatedStratifiedKFold: more independent splices => more stable estimation.
- Metric aggregator: mean / median / mean_minus_std
- Flexible flags: booster, scale_pos_weight, Early stop in search/finish.
- Artifacts: CSV leaderboard, JSON summary, best model, probabilities, business importance.
...
def build_model(config: Dict[str, Any], y_tr: np.ndarray, seed: int) -> XGBClassifier:
    ...
    # ES in Designer (for XGBoost >=2.1/3.x)
...
def sample_config(rng: np.random.RandomState,
    ...
    """Select from reasonable ranges; some things are fixed by flags."""
    if space == "light":
        depth_choices = [3,4]
    ...
    return {
        ...
        "spw_mode": spw_mode,
        "es_rounds": int(es_rounds)  # 0 = turn ON
    }
...
def cv_ap_for_config(X: np.ndarray, y: np.ndarray, config: Dict[str,Any],
    ...
    scores, rounds = [], []
    # Repeat k-fold with different seatings
...
def train_final_and_eval(X_tr: np.ndarray, y_tr: np.ndarray,
    ...
    # final training: train or train+val
    ...
    if es_rounds_final > 0:
        # internal small sample from the final set
...
def main():
    ...
    ap.add_argument("--train_on", choices=["train","trainval"], default="trainval",
                    help="what to teach the final model")
    ...
    ap.add_argument("--spw_mode", choices=["search","auto","sqrt","one"], default="search",
                    help="scale_pos_weight mode: search in auto/sqrt/one or commit")
    ap.add_argument("--booster_mode", choices=["both","gbtree","dart"], default="gbtree",
                    help="search among both boosters or fix one")
    ap.add_argument("--es_rounds", type=int, default=0,
                    help="early_stopping_rounds in search (0 = turn off)")
    ...
    ap.add_argument("--agg_alpha", type=float, default=0.5,
                    help="penalty factor for mean_minus_std")

    # финальное обучение
    ap.add_argument("--es_rounds_final", type=int, default=0,
                    help="ES on final training (0 = turn off)")
    ap.add_argument("--es_split", type=float, default=0.1,
                    help="percentage of internal validation at ES final")
    ...
    print(f"\n[OK] Leaders: {lb_path}")
    print(f"[OK] report:    {art/'metrics'/'xgb_tuning_summary.json'}")
    if args.save_model:
        print(f"[OK] model:    {art/'models'/'xgb_best.joblib'}")
    if args.save_probas:
        print(f"[OK] probas:   {art/'probas'}")
