...
python scripts/05_tune_xgb.py \
  ...
  --booster_mode gbtree --spw_mode sqrt \
  --es_rounds 0 --search_space light \
  --train_on train --save_model --save_probas
"""
...
def load_feature_names(processed_dir: str):
    ...
    if fn.exists():
        try:
            return pd.read_csv(fn, header=None).iloc[:,0].astype(str).tolist()
        except Exception:
            return None
    return None
...
def metrics_from_proba(y_true, proba, thr=0.35) -> Dict[str, float]:
    ...
    return {"PR_AUC": float(ap), "ROC_AUC": float(roc), "F1": float(f1), "Recall@Thr": float(rec_pos), "Thr": float(thr)}
...
def build_model_base(config: Dict[str, Any], y_tr: np.ndarray, seed: int) -> XGBClassifier:
    # scale_pos_weight
...
def sample_config(rng: np.random.RandomState, booster_mode: str, spw_mode_flag: str, space: str) -> Dict[str, Any]:
    if space == "light":
        depth_choices = [3,4]
    ...
    if booster_mode == "gbtree":
        ...
        booster = "dart"; rate_drop = rng.uniform(0.05, 0.3); skip_drop = rng.uniform(0.0, 0.6)
    else:
        booster = rng.choice(["gbtree","dart"])
        if booster == "dart":
            rate_drop = rng.uniform(0.05, 0.3); skip_drop = rng.uniform(0.0, 0.6)
        else:
...
def cv_ap_for_config(X: np.ndarray, y: np.ndarray, config: Dict[str,Any],
                     ...
                     agg_mode: str, agg_alpha: float, es_rounds: int) -> Tuple[float,float,float,int]:
    scores, rounds = [], []
    for rep in range(cv_repeats):
        ...
        for fold, (idx_tr, idx_va) in enumerate(skf.split(X, y), 1):
            ...
            model = build_model_base(config, y_tr, seed + 17*(rep+1) + fold)
            if es_rounds > 0:
                model.fit(X_tr, y_tr, eval_set=[(X_va, y_va)], verbose=False, early_stopping_rounds=es_rounds)
                # xgboost sklearn-API хранит best_score и best_iteration
                ...
                if best_score is None:
                    ...
                    ap = float(best_score)
                rounds.append(int(getattr(model, "best_iteration", model.get_params().get("n_estimators", 0))))
            else:
                model.fit(X_tr, y_tr)
                proba = model.predict_proba(X_va)[:,1]
                ap = average_precision_score(y_va, proba)
                rounds.append(int(model.get_params().get("n_estimators", 0)))
            scores.append(ap)
    ...
    mean_rounds = int(np.mean(rounds)) if len(rounds) else 0
    return ap_agg, ap_mean, ap_std, mean_rounds
...
def train_final_and_eval(X_tr: np.ndarray, y_tr: np.ndarray,
    ...
    if es_rounds_final > 0:
        ...
        model.fit(X_final[idx_tr], y_final[idx_tr],
                  eval_set=[(X_final[idx_va], y_final[idx_va])],
                  verbose=False, early_stopping_rounds=es_rounds_final)
    else:
    ...
    return {
        "model": model,
        "val":  metrics_from_proba(y_val, proba_val, thr=thr),
        "test": metrics_from_proba(y_te,  proba_te,  thr=thr),
        "proba_val": proba_val,
        "proba_test": proba_te
    }
...
def main():
    ...
    ap.add_argument("--train_on", choices=["train","trainval"], default="trainval",
                    help="на чём обучать финальную модель")
    ...
    for t in range(1, args.trials+1):
        cfg = sample_config(rng, booster_mode=args.booster_mode, spw_mode_flag=args.spw_mode, space=args.search_space)
        ap_agg, ap_mean, ap_std, mean_rounds = cv_ap_for_config(
            X_train, y_train, cfg,
            cv_folds=args.cv_folds, cv_repeats=args.cv_repeats, seed=args.seed+123,
            agg_mode=args.agg, agg_alpha=args.agg_alpha, es_rounds=args.es_rounds
        )
    ...
    result = train_final_and_eval(
        ...
        train_on=args.train_on, es_rounds_final=args.es_rounds_final, es_split=args.es_split, thr=args.thr
    )

    print("=== Final XGB (best CV config) ===")
    print(f"[VAL ] PR-AUC={result['val']['PR_AUC']:.4f} | ROC-AUC={result['val']['ROC_AUC']:.4f} | F1={result['val']['F1']:.4f} | Recall@{args.thr:.2f}={result['val']['Recall@Thr']:.4f}")
    print(f"[TEST] PR-AUC={result['test']['PR_AUC']:.4f} | ROC-AUC={result['test']['ROC_AUC']:.4f} | F1={result['test']['F1']:.4f} | Recall@{args.thr:.2f}={result['test']['Recall@Thr']:.4f}")

    # save artifacts
    art = pathlib.Path(args.artifacts_dir)
    ...
    eval_min = {
        ...
        "prevalence": {"train": prevalence(y_train), "val": prevalence(y_val), "test": prevalence(y_test)},
        "val": result["val"],
        "test": result["test"],
    }
    ...
    try:
        ...
        imp = pd.DataFrame(sorted(score.items(), key=lambda kv: kv[1], reverse=True), columns=["feature","importance_gain"])
        if feat_names:
            def map_name(s):
                if s.startswith("f"):
                    try:
            ...
            imp = imp[["feature_name","feature","importance_gain"]]
        imp.to_csv(art/"interpretability"/"xgb_tuned_importances.csv", index=False)
    except Exception:
        pass

    print(f"\n[OK] leaderboard : {lb_path}")
    print(f"[OK] summary     : {art/'metrics'/'xgb_tuning_summary.json'}")
    print(f"[OK] eval (min)  : {art/'metrics'/'xgb_tuned_eval.json'}")
    if args.save_model:
        print(f"[OK] model       : {art/'models'/'xgb_tuned.joblib'}")
    if args.save_probas:
        print(f"[OK] preds       : {art/'preds'}")
    print(f"[OK] importances : {art/'interpretability'/'xgb_tuned_importances.csv'}")
