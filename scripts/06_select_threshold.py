...
Threshold matching for XGBoost by Fβ using OOF predictions (Repeated Stratified CV),
then final training and assessment on the test at the threshold found.

require: artifacts/metrics/xgb_tuning_summary.json (из 05_tune_xgb.py), data/processed/* из 02_preprocess_split.py.
...
try:
    ...
    raise SystemExit("xgboost not found: " + str(e))
...
def load_best_config(summary_path: pathlib.Path) -> Dict[str, Any]:
    ...
    if not cfg:
        raise SystemExit("No field in file 'best_config'. Make sure 05_tune_xgb.py has worked.")
    return cfg
...
def build_model(cfg: Dict[str, Any], y_tr: np.ndarray, seed: int) -> XGBClassifier:
    ...
    # support dart parameters when needed
    if params["booster"] == "dart":
        params.update(
            rate_drop=float(cfg.get("rate_drop", 0.1)),
            skip_drop=float(cfg.get("skip_drop", 0.1))
        )
    # No ES here: threshold is selected on OOF, ES prevents on small rolls
...
def fbeta_score_pos(y_true, y_hat, beta=2.0):
    # only consider precision/recall for positive class
...
def main():
    ...
    ap.add_argument("--beta", type=float, default=2.0, help="β for Fβ (recall priority at β>1)")
    ap.add_argument("--thr_grid", default="0.01:0.60:0.005", help="min:max:step search threshold")
    ...
    ap.add_argument("--train_on", choices=["train","trainval"], default="trainval",
                    help="what we teach the final model after threshold selection")
    ...
    assert oof_mask.all(), "Not all OOF values are counted; check CV settings."
    oof_ap = average_precision_score(y_train, oof_proba)
    print(f"[INFO] OOF PR-AUC on train = {oof_ap:.4f}")

    # --- threshold search by Fβ
    ...
    # --- final training and assessment on the test at the threshold found
    ...
    print(f"\n[OK] results saved: {art/'metrics'/'xgb_thresholded_eval.json'}")
    if args.save_curves:
        print(f"[OK] PR curves saved in: {art/'metrics'}")
